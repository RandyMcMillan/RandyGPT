...
fn main() -> std::io::Result<()> {
    ...
    unsafe {
        ...
        } else if (&*(&raw const BPE_VOCAB_PATH)).is_empty() {
    ...
    unsafe {
        ...
        println!("Model: {} — {} layers, {} heads, {}-dim", model_size_name, *(&raw const N_LAYER), *(&raw const N_HEAD), *(&raw const N_EMBD) );
        println!("Block size: {}, Vocab size: up to {}", *(&raw const BLOCK_SIZE), *(&raw const MAX_VOCAB) );
        println!();
    }
    ...
    debug!("BPE vocab path: {}, exists: {}", (*(&raw const BPE_VOCAB_PATH)).as_str(), have_bpe_vocab);

    let (tokenizer, data, val_data) = if have_token_cache && have_bpe_vocab {
        // Fast path: load vocab + cached tokens, skip raw text entirely
        println!("Loading BPE vocab from {}...", (*(&raw const BPE_VOCAB_PATH)).as_str());
        debug!("Fast path: Loading BPE vocab from {}.", (*(&raw const BPE_VOCAB_PATH)).as_str());
        let tokenizer = Tokenizer::load_bpe((*(&raw const BPE_VOCAB_PATH)).as_str())
            .map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e.to_string()))?;
        ...
        let tokenizer = if let Some(target) = bpe_vocab_size {
            if Path::new((*(&raw const BPE_VOCAB_PATH)).as_str()).exists() {
                println!("Loading BPE vocab from {}...", (*(&raw const BPE_VOCAB_PATH)).as_str());
                debug!("Attempting to load BPE vocab from {}.", (*(&raw const BPE_VOCAB_PATH)).as_str());
                match Tokenizer::load_bpe((*(&raw const BPE_VOCAB_PATH)).as_str()) {
                    Ok(t)  => { println!("Loaded BPE vocab ({} tokens)", t.vocab_size); debug!("Loaded BPE vocab with {} tokens.", t.vocab_size); t }
                    Err(e) => {
                        eprintln!("Failed to load {}: {}. Retraining...", (*(&raw const BPE_VOCAB_PATH)).as_str(), e);
                        warn!("Failed to load BPE vocab {}: {}. Retraining.", (*(&raw const BPE_VOCAB_PATH)).as_str(), e);
                        let t = Tokenizer::from_text_bpe(&training_text, target);
                        t.save_bpe((*(&raw const BPE_VOCAB_PATH)).as_str())?;
                        println!("BPE vocab ({} tokens) saved to {}", t.vocab_size, (*(&raw const BPE_VOCAB_PATH)).as_str());
                        debug!("Retrained and saved BPE vocab with {} tokens to {}.", t.vocab_size, (*(&raw const BPE_VOCAB_PATH)).as_str());
                        t
                    }
                }
            } else {
                ...
                t.save_bpe((*(&raw const BPE_VOCAB_PATH)).as_str())?;
                println!("BPE vocab ({} tokens) saved to {}", t.vocab_size, (*(&raw const BPE_VOCAB_PATH)).as_str());
                debug!("New BPE vocab ({} tokens) saved to {}.", t.vocab_size, (*(&raw const BPE_VOCAB_PATH)).as_str());
                t
            }
        } else {
